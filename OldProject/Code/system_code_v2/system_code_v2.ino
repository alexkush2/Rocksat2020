...
int pump = 3;                          // Define pump by pin 3
...
//pinMode(input_accelerometer_x, INPUT);           // declare accelerometer x axis as input
  ...
  servoLeft.attach(12);                            // Track Lock Servo
  servoRight.attach(13);                           // Retraction Servo
 ...
 //******************************************************************************************
 //Once accelerometer reads a signal from the y axis (aka the rocket has launched,
...
void loop() {
  for (sample = 1; sample < 7; sample++) { //for the ascend - total ascend time is XX.X seconds
    //retract_reverse();             // send signal to servo that retracts solenoid
    //delay(300);
