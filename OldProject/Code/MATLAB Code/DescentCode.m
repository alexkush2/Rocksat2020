...
%Importing RockSat 2011 Radar Data for the Descent.
%% Initialize variables.
filename = 'C:\Users\Aprt 2F\Documents\MATLAB\RadarData_Descent.txt';
delimiter = '\t';
...
t=dataArray{:, 1}; %Time, s.  Begins at 295.1s
Z=dataArray{:, 2}; %Altitude, m.  Begins at 46901m
V1=dataArray{:, 3}; %Velocity, m/s.  Begins at -1117 m/s
V1=abs(V1);
...
%%
%Calculating Dynamic Pressure (q) at Dynamic pressure port using
%Static and Stagnation Pressures of flow.

qset=zeros(1,length(Z));
for jj=1:1:length(Z)
    qset(jj)=po3set(jj)-p3set(jj);
    if qset==max(qset)
    display(Z(jj))
    break
    end
